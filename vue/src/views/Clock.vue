...
<!--      <el-button type="primary" @click="handleAdd">新增 <i class="el-icon-circle-plus-outline"></i></el-button>-->
      ...
      <!-- <el-upload action="http://localhost:9090/clock/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
...
export default {
  ...
  methods: {
    ...
    save() {
      this.request.post("/clock", this.form).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/clock/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/clock/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/clock/export")
    },
